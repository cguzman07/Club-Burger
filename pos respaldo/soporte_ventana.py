...
def ventana_caja(usuario, ventana_principal):
    ...
    def actualizar_carrito():
        ...
        def imprimir_factura():
            ...
            try:
                ...
                imprimir_ticket_real(carrito, total, metodo_pago.get(), Dinero_recibido.get(), cambio)
                registrar_venta()
                carrito.clear()
                total_var.set(0.0)
                actualizar_carrito()
        ...
        for item in carrito:
            ...
            tk.Label(frame_carrito, text=f"{nombre} x{cantidad} - ${precio * cantidad:,.2f}", bg="#cdf5d0").pack(anchor="w", padx=10)
        ...
        tk.Label(frame_carrito, text="Dinero recibido:", bg="#cdf5d0").pack(padx=10, anchor="w")
        Dinero_recibido = ttk.Combobox(frame_carrito, values=["10000", "20000", "30000", "40000","50000", "60000","70000","80000", "90000", "100000"], font=BASE_FONT)
        Dinero_recibido.pack(padx=10, fill="x")
        Dinero_recibido.current(0)


        cambio_var = tk.StringVar()
        tk.Label(frame_carrito, textvariable=cambio_var, bg="#cdf5d0", fg="black", font=BASE_FONT).pack(pady=(5,0))

        def calcular_cambio():
            try:
                recibido = float(Dinero_recibido.get())
                cambio = recibido - total_var.get()
                cambio_var.set(f"Cambio: ${cambio:,.2f}")
            except ValueError:
                cambio_var.set("Monto inválido")

        styled_button(frame_carrito, text="Calcular cambio", bg_color="#1976D2", command=calcular_cambio).pack(padx=10, fill="x", pady=5)
    ...
    def registrar_venta():
        try:
            ...
            # Usar una conexión para todo el bloque de venta
            conn = get_connection()
            cur = conn.cursor()
            try:
